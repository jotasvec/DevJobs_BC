...
import { SeekerProfileModel } from "../models/seekerProfile.js";
...
export class SeekerProfileController {
  static getOwn: RequestHandler = (req, res, next) => {
    try {
      ...
      const profile = SeekerProfileModel.getByUserId(userId);
  ...
  static updateOwn: RequestHandler = (req, res, next) => {
    try {
      ...
      SeekerProfileModel.upsert(userId, req.body);
  ...
  static getByUserId: RequestHandler<{ userId: string }> = (req, res, next) => {
    try {
      const profile = SeekerProfileModel.getByUserId(req.params.userId);
