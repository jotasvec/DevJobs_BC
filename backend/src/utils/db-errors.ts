...
export const handleDBError = (error : unknown): never => {
    if (error instanceof Error) {
        ...
        //SQLite Logic
        if (err.code === 'SQLITE_CONSTRAINT_UNIQUE') throw new Error(ERROR_CODES.DUPLICATED_TECHNOLOGY);
        if (err.code === 'SQLITE_CONSTRAINT_FOREIGNKEY') throw new Error(ERROR_CODES.INVALID_CATEGORY);
